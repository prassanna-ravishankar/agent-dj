...
export function buildScenario(id: ScenarioId, now: number = Date.now()): Snapshot {
  ...
  return {
    ...
    conductor: {
      ...
      pid: runtime.running ? 48231 : null,
      local_only: true,
      hosted_tokens: false,
      set: runtime.running
        ? {
            status: 'running',
            brief: 'modern Indian house fusion, warm and percussive, patient but alive',
            duration_minutes: 90,
            started_at: iso(now - 27 * 60_000),
            ends_at: iso(now + 63 * 60_000),
            phase: 'rise',
            progress: 0.3,
            current_direction: 'Indian house fusion, supple tabla, warm sub, rising melodic answers',
            next_direction: 'confident forward motion, denser hand percussion, memorable modal hook',
            current_note: 'Rise — supple tabla, warm sub, rising melodic answers',
            next_note: 'Crest — denser hand percussion and a memorable modal hook',
            next_cue_at: iso(now + 4 * 60_000),
            last_steering: 'bring the hand percussion forward, keep the synths still',
            cue_index: 4,
            local_only: true,
            hosted_tokens: false,
            activity: 'ready',
        ...
        : {
            status: 'idle', brief: '', duration_minutes: 90, started_at: null, ends_at: null,
            phase: 'idle', progress: 0, current_direction: null, next_direction: null,
            current_note: null, next_note: null,
            next_cue_at: null, last_steering: null, cue_index: 0, local_only: true,
            hosted_tokens: false,
            activity: 'idle',
          },
    },
